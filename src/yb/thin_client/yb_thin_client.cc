...
struct ybthin_table {
  ...
  // Tablet start keys, ascending, first empty. Needed to fan a backward range scan out over
  // tablets. Captured at open; a later split leaves it stale until the table is reopened.
...
constexpr size_t kPagingHeaderLen = 2 + 4 + 4 + 8 + 8 + 4;
// Tablet index for a fanned-out backward range scan; kNoTablet for every other scan.
constexpr uint32_t kNoTablet = 0xffffffffu;
...
std::string WrapPagingState(
    ...
    uint32_t tablet_index, const std::string& srv) {
  std::string out;
...
Result<PinInfo> UnwrapPagingState(const uint8_t* data, size_t len) {
  ...
  return PinInfo{
      ...
      GetU32LE(data + 26), Slice(data + kPagingHeaderLen, len - kPagingHeaderLen)};
}
...
// A write addresses exactly one row, so its partition key is that row's encoded key.
Status RouteWrite(const ybthin_table& table, yb::PgsqlWriteRequestPB* write) {
  if (table.schema.num_hash_key_columns() > 0) {
    auto partition_key = VERIFY_RESULT(
        table.partition_schema.EncodePgsqlHash(write->partition_column_values()));
    write->set_hash_code(PartitionSchema::DecodeMultiColumnHashValue(partition_key));
    write->set_partition_key(std::move(partition_key));
    return Status::OK();
  }
  write->set_partition_key(VERIFY_RESULT(
      dockv::GetRangePartitionKey(table.schema, write->range_column_values())));
  return Status::OK();
}
...
void SetTabletBounds(const ybthin_table& table, size_t i, yb::PgsqlReadRequestPB* read) {
  ...
  if (i > 0) {
    const auto& lower = parts[i];
    if (!read->has_lower_bound() || read->lower_bound().key() < lower) {
      read->mutable_lower_bound()->set_key(lower);
      read->mutable_lower_bound()->set_is_inclusive(true);
    }
  }
  if (i + 1 < parts.size()) {
    const auto& upper = parts[i + 1];
    if (!read->has_upper_bound() || read->upper_bound().key() > upper) {
      read->mutable_upper_bound()->set_key(upper);
...
Status RouteRead(
  ...
  if (!read->partition_column_values().empty()) {
    read->set_partition_key(VERIFY_RESULT(
        table.partition_schema.EncodePgsqlHash(read->partition_column_values())));
  }
  const bool range_sharded = table.schema.num_hash_key_columns() == 0;
  // A backward range scan cannot page across tablets on this release: the tserver derives its
  // partition key from the upper bound alone (client/yb_op.cc
  // IsPartitionsChangeDependantBackwardScan), ignoring both our key and the paging state, so it
  // would re-route to the last tablet forever. pggate sidesteps this by scanning one tablet per op
  // (PgDocReadOp::SetLowerUpperBound); the shim does the same, walking tablets high to low and
  // carrying the current one in the paging state.
  if (range_sharded && !read->is_forward_scan() && tablet_index != kNoTablet) {
    SetTabletBounds(table, tablet_index, read);
    return Status::OK();
  }
  const auto& paging = read->paging_state();
  const bool continuation = read->has_paging_state() && paging.has_next_partition_key();
  if (continuation) {
...
void FinishRead(ReadCall* raw) {
  ...
  for (size_t i = 0; i < n_ops; ++i) {
    ...
    // A missing paging_state means this op's scan is exhausted -- except for a fanned-out backward
    // range scan, which then moves down to the previous tablet and starts a fresh page there.
    // Otherwise wrap the server's state with the pinning header so a continuation comes back to
    // this batch's session.
    const uint32_t tablet = call->op_tablet[i];
    std::string srv;
    bool emit = false;
    uint32_t next_tablet = tablet;
    ...
    if (emit) {
      std::string wrapped = WrapPagingState(
          call->session_index, call->generation, call->read_time_serial, call->txn_serial,
          next_tablet, srv);
...
ybthin_status ybthin_table_open(
  ...
  for (const auto& key : resp.partitions().keys()) {
    table->partitions.push_back(key);
  }
  if (!s.ok()) return FromStatus(s);
...
void ybthin_read_async(
  ...
  std::vector<uint32_t> pinned_tablet(n_ops, kNoTablet);
  for (size_t i = 0; i < n_ops; ++i) {
    const auto& op = ops[i];
    ...
    pinned_tablet[i] = pin->tablet_index;
  }
  if (!have_pinned) {
  ...
  // One snapshot for the whole batch. On this release the read-time fields are flat on
  // PgPerformOptionsPB; later builds fold them into a nested ReadTimeOptionsPB.
  auto* rto = req.mutable_options();
  if (read_time_ht == 0) {
...
void ybthin_upsert_batch_async(
  ...
  bool dispatch = false;
  {
    ...
    if (!open.ok()) {
      ...
      SetReadTimeSerial(raw->req.mutable_options(), s->read_time_serial++);
