...
void ybthin_read_async(
  ...
  // One snapshot for the whole batch. On this release the read-time fields live directly on
  // PgPerformOptionsPB (they were folded into a nested ReadTimeOptionsPB only on later builds), so
  // set them on the options message itself.
  auto* rto = req.mutable_options();
  if (read_time_ht == 0) {
    // Fresh read point for this batch, matching yb_tsclient's plain-read wire exactly: the proxy
    // picks a current, clamped read time at setup and files it under our (advancing) read serial.
    // ENSURE + clamp + an advancing read serial + a windowed history_min (below) + a fresh
    // txn_serial_no (at dispatch) are ALL required -- dropping any leaves the session's read point
    // sticky, so later reads never see rows committed after the first.
...
void ybthin_upsert_batch_async(
  ...
  bool dispatch = false;
  {
    ...
    if (!open.ok()) {
      ...
      auto* rto = raw->req.mutable_options();  // read-time fields are flat on this release
